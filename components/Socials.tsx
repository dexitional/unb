...
function Socials({ siteUrl,title,slug }: any) {
  return (
    <div className="flex flex-1 flex-wrap items-center gap-2">
        ...
        <Link href={`https://api.whatsapp.com/send?text=${encodeURI(title)}%20%0A%0A%20${encodeURI(siteUrl+'/'+slug)}`}><FaWhatsapp className="p-3 w-10 h-10 bg-blue-600 text-white rounded-lg cursor-pointer"/></Link>
    </div>
  )
}
